...
<!-- Formulario jugador -->
<div class="row">
<a href="main.php"><h2><img src="ico/android-chrome-192x192.png" width="76px" /></a> Asistencia</h2>
  ...
  <div class="card-block">
    <h4 class="card-title">Martes</h4>
		<div class="btn-group-lg" data-toggle="buttons">
			  <button class="btn btn-outline-success <?php disponibilidad_actual("active", 1, 2) ?>" <?php echo $ses_1; ?>>
				<input name="dia1" id="si" autocomplete="off" value="2" type="radio" <?php disponibilidad_actual("checked", 1, 2) ?>> Si
			  </button>
			  <button class="btn btn-outline-warning <?php disponibilidad_actual("active", 1, 1) ?>" <?php echo $ses_1; ?>>
				<input name="dia1" id="duda" autocomplete="off" value="1" type="radio" <?php disponibilidad_actual("checked", 1, 1) ?>> Duda
			  </button>
			  <button class="btn btn-outline-danger <?php disponibilidad_actual("active", 1, 0) ?>" <?php echo $ses_1; ?>>
				<input name="dia1" id="no" autocomplete="off" value="0" type="radio" <?php disponibilidad_actual("checked", 1, 0) ?>> No
			  </button>
		</div>
	<h4 class="card-title">Miércoles</h4>
		<div class="btn-group-lg" data-toggle="buttons">
			  <button class="btn btn-outline-success <?php disponibilidad_actual("active", 2, 2) ?>" <?php echo $ses_2; ?>>
				<input name="dia2" id="si" autocomplete="off" value="2" type="radio" <?php disponibilidad_actual("checked", 2, 2) ?>> Si
			  </button>
			  <button class="btn btn-outline-warning <?php disponibilidad_actual("active", 2, 1) ?>" <?php echo $ses_2; ?>>
				<input name="dia2" id="duda" autocomplete="off" value="1" type="radio" <?php disponibilidad_actual("checked", 2, 1) ?>> Duda
			  </button>
			  <button class="btn btn-outline-danger <?php disponibilidad_actual("active", 2, 0) ?>" <?php echo $ses_2; ?>>
	...
	<h4 class="card-title">Viernes</h4>
		<div class="btn-group-lg" data-toggle="buttons">
			  <button class="btn btn-outline-success <?php disponibilidad_actual("active", 3, 2) ?>" <?php echo $ses_3; ?>>
				<input name="dia3" id="si" autocomplete="off" value="2" type="radio" <?php disponibilidad_actual("checked", 3, 2) ?>> Si
			  </button>
			  <button class="btn btn-outline-warning <?php disponibilidad_actual("active", 3, 1) ?>" <?php echo $ses_3; ?>>
				<input name="dia3" id="duda" autocomplete="off" value="1" type="radio" <?php disponibilidad_actual("checked", 3, 1) ?>> Duda
			  </button>
			  <button class="btn btn-outline-danger <?php disponibilidad_actual("active", 3, 0) ?>" <?php echo $ses_3; ?>>
	...
	<!-- Fin mostrar partidos -->
		<div class="btn-group-lg" data-toggle="buttons">
			  <button class="btn btn-outline-success <?php disponibilidad_actual("active", 4, 2) ?>" <?php echo $ses_4; ?>>
				<input name="dia4" id="si" autocomplete="off" value="2" type="radio" <?php disponibilidad_actual("checked", 4, 2) ?>> Si
			  </button>
			  <button class="btn btn-outline-warning <?php disponibilidad_actual("active", 4, 1) ?>" <?php echo $ses_4; ?>>
				<input name="dia4" id="duda" autocomplete="off" value="1" type="radio" <?php disponibilidad_actual("checked", 4, 1) ?>> Duda
			  </button>
			  <button class="btn btn-outline-danger <?php disponibilidad_actual("active", 4, 0) ?>" <?php echo $ses_4; ?>>
	  ...
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			<strong>M: </strong> <?php echo mysqli_num_rows(mysqli_query($_SESSION['con'], "SELECT `id` FROM ".$tabla_asistencia." WHERE `asist_1` = 2 AND `semana` = ".$semana->id." AND `usuario_equipo` = ".$_SESSION["equipo"].""));?> ( <?php echo mysqli_num_rows(mysqli_query($_SESSION['con'], "SELECT `id` FROM ".$tabla_asistencia." WHERE `asist_1` = 1 AND `semana` = ".$semana->id));?>)<br>
			<strong>X: </strong> <?php echo mysqli_num_rows(mysqli_query($_SESSION['con'], "SELECT `id` FROM ".$tabla_asistencia." WHERE `asist_2` = 2 AND `semana` = ".$semana->id." AND `usuario_equipo` = ".$_SESSION["equipo"].""));?> ( <?php echo mysqli_num_rows(mysqli_query($_SESSION['con'], "SELECT `id` FROM ".$tabla_asistencia." WHERE `asist_2` = 1 AND `semana` = ".$semana->id));?>)<br>
			<strong>V: </strong> <?php echo mysqli_num_rows(mysqli_query($_SESSION['con'], "SELECT `id` FROM ".$tabla_asistencia." WHERE `asist_3` = 2 AND `semana` = ".$semana->id." AND `usuario_equipo` = ".$_SESSION["equipo"].""));?> ( <?php echo mysqli_num_rows(mysqli_query($_SESSION['con'], "SELECT `id` FROM ".$tabla_asistencia." WHERE `asist_3` = 1 AND `semana` = ".$semana->id));?>)<br>
			<strong>P: </strong> <?php echo mysqli_num_rows(mysqli_query($_SESSION['con'], "SELECT `id` FROM ".$tabla_asistencia." WHERE `asist_4` = 2 AND `semana` = ".$semana->id." AND `usuario_equipo` = ".$_SESSION["equipo"].""));?> ( <?php echo mysqli_num_rows(mysqli_query($_SESSION['con'], "SELECT `id` FROM ".$tabla_asistencia." WHERE `asist_4` = 1 AND `semana` = ".$semana->id));?>)
			...
			?>
					 ...
					 <p><div class="btn-group" role="group" aria-label="Asistencia">
						...
						<button type="button" class="btn <?php checkasist($seleccionada->asist_1); ?>">M</button>
						<button type="button" class="btn <?php checkasist($seleccionada->asist_2); ?>">X</button>
						<button type="button" class="btn <?php checkasist($seleccionada->asist_3); ?>">V</button>
						<button type="button" class="btn <?php checkasist($seleccionada->asist_4); ?>">P</button>
					</div></p>
